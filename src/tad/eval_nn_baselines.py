...
def run_nn_eval(module_path, dataset_name, eval_method="standard", verbose=False):
	sota_index = [
		"1-Layer MLP",
	...
	sota = ["mlp", "MLPmixer", "Transformer", "gcn"]
	...
	for model_name in sota:
		...
		if dataset_name == "SMD":
			gt_path = Path(
				module_path,
				"resources",
				"NN_baselines_predictions",
				model_name + "_" + dataset_name.lower() + "_gt.pkl",
			)
			score_path = Path(
				module_path,
				"resources",
			...
			for i in range(len(gt_dict)):
				res_i, dfs_i = evaluate_ts(
					score_dict[i], gt_dict[i], eval_method=eval_method, verbose=verbose
				)
			...
			arr = np.load(
				Path(
					...
					"resources",
					"NN_baselines_predictions",
					model_name + "_" + dataset_name.lower() + ".npy",
				)
			)
			scores = arr[0]
			gt_labels = arr[1]

			results_i, df_i = evaluate_ts(
				scores, gt_labels, eval_method=eval_method, verbose=verbose
			)
...
def evaluate_nn_baselines(
	...
	for dataset_name in dataset_names:
		...
		result, df = run_nn_eval(
			root_path, dataset_name, eval_method=eval_method, verbose=False
		)
