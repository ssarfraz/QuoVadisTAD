...
from tad import DEFAULT_DATA_DIR, DEFAULT_RESOURCE_DIR
...
def read_data(dataset: GeneralDataset,
              dataset_trace=None,
    ...
    if type(trainset) is list:
        if dataset_trace is not None:
            ...
            print(f' using first trace of this dataset. Specify dataset_trace otherwise')
...
def conv_blk(inputs, filters,
    kernel_size,
    strides=(1, 1),
    activation=None,
    BN=True):
    
    conv_out = layers.Conv2D(filters, kernel_size,
                             strides=strides,
                             padding="same",
...
def build_conv_model(input_shape, training=True):
    ...
    conv_out = layers.MaxPooling2D(pool_size=(2, 2), strides=None, padding="valid")(conv_out)
    conv_out = conv_blk(conv_out, 64, 3, strides=(1,1), activation='relu', BN=False)
    conv_out = layers.MaxPooling2D(pool_size=(2, 2), strides=None, padding="valid")(conv_out)
    
    conv_out = conv_blk(conv_out, input_shape[1], 3, strides=(1,1), activation='relu', BN=False)
...
def sequense_sensor_error_model(input_shape, config=None, training=True):
    ...
    #seq_trend_leaner
    inputs_seq = layers.Dense(embedd_1, activation="relu")(inputs_seq) #(batch, nodes, embedding_dim)
    ...
    regression_node = layers.Dense(embedd_2, activation="relu")(inputs_nodes) #(batch, seq, embedding_dim)
    ...
    regression_embeddings = layers.Concatenate()([regression_seq, regression_node])  #Add()
    regression_embeddings = layers.Reshape((1, 2 *lstm_units))(regression_embeddings) #(batch, 1, embedding_dim)
    regression_targets = layers.Dense(num_nodes, activation=None)(regression_embeddings)
    
    if not training: 
        model = keras.Model(inputs=inputs_nodes, outputs=[regression_targets, regression_embeddings])
    else:
...
def gcn_sequense_sensor_error_model(input_shape, config=None, training=True):
    ...
    inputs_seq = layers.Reshape((num_nodes, num_seq, 1))(inputs)    #(batch, nodes, seq, 1)
    
    outputs_nodes = gcn_node(inputs_nodes) #output shape (batch_size, output_dim_1 x num_nodes]
    outputs_seq = gcn_seq(inputs_seq)    #output shape (batch_size, output_dim_2 x num_seq]
    ...
    if not training: 
        model = keras.Model(inputs=inputs, outputs=[regression_targets, regression_embeddings])
    else:
        model = keras.Model(inputs=inputs, outputs=regression_targets)
    return model 
...
def gscn_lstm_recons_model(input_shape, config=None, training=True):
    ...
    embedd_1 = config['embedding_dim'][0]   # gcn out_dim for each sensor output some embedding of this size  
    ...
    regression_targets = gcn_seq(inputs_nodes) #output shape (batch_size, seq, lstm_units]
          
    regression_targets = layers.Dense(num_nodes)(regression_targets)  # dot his if lstm_units are not num_nodes
...
def build_sequence_embedder(input_shape=(None, None),
    ...
    if model == "Transformer":
        ...
        blocks = keras.Sequential(
            [FNetLayer(num_seq, embedding_dim, dropout_rate) for _ in range(num_blocks)],
            name="representation"
        )
    elif model == "MLPMixer":
        blocks = keras.Sequential(
        ...
        return sequense_sensor_error_model(input_shape, config=config, training=training)
    
    elif model == "gcn_seq_sensor_model":
        return gcn_sequense_sensor_error_model(input_shape, config=config, training=training)
        ...
        raise ValueError('model type not Implemented. use one of {Transformer, MLPMixer, FNet, MLP_simple}')    
...
def get_model(input_shape,
    ...
    if config_data['optimizer'] == "adam":
        optim = keras.optimizers.legacy.Adam(learning_rate=config_data['learning_rate']) # switched to legacy
    
    
    elif config_data['optimizer'] == "adamw":
        # Create Adam optimizer with weight decay.
        optim = tfa.optimizers.AdamW(
            learning_rate=config_data['learning_rate'], weight_decay=config_data['weight_decay'],
        )
    elif config_data['optimizer'] == "rmsprop":
        optim = keras.optimizers.legacy.RMSprop(learning_rate=config_data['learning_rate']) # switched to legacy
    else:
        optim = keras.optimizers.legacy.SGD(learning_rate=config_data['learning_rate'], momentum=0.9) # switched to legacy
...
def train_embedder(dataset: GeneralDataset,
                   dataset_trace=None,
                   config_path='./conifgs/..',
                   load_weights=False,
                   training=True
    ...
    if dataset_trace is None:
        checkpoint_folder = Path(DEFAULT_RESOURCE_DIR, 'model_checkpoints', dataset[0], config_name.split('.')[0])
    else:
        checkpoint_folder = Path(DEFAULT_RESOURCE_DIR, 'model_checkpoints', dataset[0], str(dataset_trace), config_name.split('.')[0])
    
    checkpoint_folder.mkdir(parents=True, exist_ok=True)
    
    # read dataset
    trainset, valset, testset, _ = read_data(dataset,
...
def test_embedder(dataset: GeneralDataset,
                  dataset_trace=None,
   ...
   # handle missing key in old configs, set if doesnt exist
    ...
    if dataset_trace is None:
        checkpoint_folder = Path(DEFAULT_RESOURCE_DIR, 'model_checkpoints', dataset_name, config_name.split('.')[0])
    else:
        checkpoint_folder = Path(DEFAULT_RESOURCE_DIR, 'model_checkpoints', dataset_name, str(dataset_trace), config_name.split('.')[0])
    
    #checkpoint_folder.mkdir(parents=True, exist_ok=True)
    print(f'check_path: {checkpoint_folder}')
    # read dataset
    trainset, _, testset, labels = read_data(dataset,
                                             dataset_trace=dataset_trace,
                                             preprocess=config_data["preprocess"],
                                             config=config_data)
