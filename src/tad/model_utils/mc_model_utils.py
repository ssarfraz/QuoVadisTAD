...
class MCDropOutLayer(layers.Layer):
	...
	def call(self, inputs):
		input_, train = inputs
		dropouts_ = [
			layers.Dropout(self.mc_rate)(input_, training=train)
...
def mc_drop_out_layer(embeddings, mc_rate=0.5, mc_steps=10):
	...
	dropouts_ = [
		layers.Dropout(mc_rate)(embeddings, training=True) for i in range(mc_steps)
	]
...
def load_model_mc(pretrained_model, input_shape=None, config=None):
	...
	dropouts_ = MCDropOutLayer(config["mc_dropout_rate"], config["mc_steps"])(
		[embeddings, True]
	)
	dropouts_ = [embeddings] + dropouts_
	...
	mc_regression_preds = tf.stack(
		dropouts_, axis=1
	)  # shape [batch_size, mc_stpes, 1, sensors]
	mc_regression_preds = layers.Dense(input_shape[1])(mc_regression_preds)

	return keras.Model(
		inputs=pretrained_model.input, outputs=[mc_regression_preds, embeddings]
	)
