...
def convert_cluster_labels_to_scores(labels, test_labels): # TODO add docstring and / or make private or remove
    ...
    pr, rec, f1, _ = metrics.precision_recall_fscore_support(test_labels, pred, average='binary')
    score_norm = np.random.uniform(low=0., high=0.5, size=len(pred))
...
def assign_cluster_labels():# TODO add docstring and / or make private or remove
    ...
    d_proto = metrics.pairwise.pairwise_distances(test_cluster_centers, train_array_center, metric='cosine')
