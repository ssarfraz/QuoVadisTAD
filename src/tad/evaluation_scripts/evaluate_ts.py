...
def evaluate_ts(scores, targets, eval_method="standard", verbose=True):
	...
	if eval_method == "point_adjust":
		# point adjust eval
		results, df = evaluate_pa(
			scores, targets, pa=True, interval=10, k=0, verbose=False
		)
		...
		results, df = get_ts_eval(
			scores, targets, eval_method=eval_method, verbose=verbose
		)
