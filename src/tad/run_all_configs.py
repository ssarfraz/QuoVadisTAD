	...
	for config_path in configs:
		...
		if config_to_run is not None:
			if config_name != config_to_run:
				print(
					f"Skipping....: Trained Model on {dataset_name} already exist for configuration {config_name}. To retrain specify overwrite True or provide config name with --config-to-run param."
				)
				continue
			else:
				overwrite = True
			...
			if os.path.exists(checkpoint):
				if not overwrite and not load_weights:
					print(
						f"Skipping....: Trained Model on {dataset_name} already exist for configuration {config_name}. To retrain specify overwrite True."
					)
					continue
...
def run_configs_trace(
	...
	dataset = GeneralDataset.__members__[
		dataset_name.upper()
	]  # for consistency with all the other scripts
