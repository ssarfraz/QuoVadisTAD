...
def _load_wadi(
	data_path: Union[str, Path] = DEFAULT_DATA_DIR / "WADI",
...
def _load_wadi_gdn(
	data_path: Union[str, Path] = DEFAULT_DATA_DIR / "WADI_gdn",
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
	if not isinstance(data_path, Path):
		data_path = Path(data_path)
...
def _load_swat(
	data_path: Union[str, Path] = DEFAULT_DATA_DIR / "SWaT",
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
	if not isinstance(data_path, Path):
		data_path = Path(data_path)
...
def _load_ucr(
	data_path: Union[str, Path] = DEFAULT_DATA_DIR / "UCR",
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
	if not isinstance(data_path, Path):
		data_path = Path(data_path)
...
def _load_ucr_all(
	...
	data_traces = list(
		set(
			[
				f'{filename.split("_")[0]}_'
				for filename in find_files_in_path(directory=files_path)
			]
		)
	)
...
def _load_smd(
	data_path: Union[str, Path] = DEFAULT_DATA_DIR / "SMD",
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
	# load all machines subsets as in orignal OmniAnomly paper and Kime et al"Towards a Rigorous Evaluation of Time-series Anomaly Detection" 2022 paper
	if not isinstance(data_path, Path):
		data_path = Path(data_path)
...
def _load_msl(
	data_path: Union[str, Path] = DEFAULT_DATA_DIR / "MSL",
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
	if not isinstance(data_path, Path):
		data_path = Path(data_path)
	...
	assert (
		np.concatenate(data[0]).shape[1]
		== np.concatenate(data[1]).shape[1]
		== np.concatenate(data[2]).shape[1]
		== 55
	)
...
def _load_smap(
	data_path: Union[str, Path] = DEFAULT_DATA_DIR / "SMAP",
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
	if not isinstance(data_path, Path):
		data_path = Path(data_path)
...
dataset_loader_map: Dict[
	GeneralDataset, Callable[[str | Path], Tuple[np.ndarray, np.ndarray, np.ndarray]]
] = {
