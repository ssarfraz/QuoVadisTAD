...
from tad.utils.data_utils import (
	preprocess_data,
	median_iqr_norm,
	concatenate_windows_feat,
)
...
def run_baselines(
	...
	if isinstance(train, list):
		...
		for i in range(len(train)):
			tr, _, te = preprocess_data(
				train[i], test[i], 1.0, 0.0, normalization=preprocessing
			)
		...
		train, _, test = preprocess_data(
			train, test, 0.98, 0.02, normalization=preprocessing
		)
...
def evaluate_baselines(
	...
	results_0, df_0 = evaluate(
		np.random.rand(test_labels.shape[0]),
		test_labels,
		eval_method=eval_method,
		verbose=verbose,
	)
	...
	results_01, df_01 = evaluate(
		ano_sens_cand > 0, test_labels, eval_method=eval_method, verbose=verbose
	)

	# Baseline-2: closest distance to train timestamp
	scores = metrics.pairwise.pairwise_distances(
		test_array, train_array, metric=distance
	)
	results_1, df_1 = evaluate(
		scores.min(1), test_labels, eval_method=eval_method, verbose=verbose
	)

	# Baseline 3: difference to train dist mean
	scores = metrics.pairwise.pairwise_distances(
		test_array, train_array.mean(0)[None, :], metric=distance
	)
	results_2, df_2 = evaluate(
		scores, test_labels, eval_method=eval_method, verbose=verbose
	)
	...
	results_3, df_3 = evaluate(
		median_iqr_norm(np.abs(inv_pca - test_array)).max(1),
		test_labels,
		eval_method=eval_method,
		verbose=verbose,
	)
	# mean-std normalization on deviation scores
	results_3_1, df_3_1 = evaluate(
		median_iqr_norm(np.abs(inv_pca - test_array), norm="mean-std").max(1),
		test_labels,
		eval_method=eval_method,
		verbose=verbose,
	)
	...
	results_4, df_4 = evaluate(
		np.abs(inv_pca - test_array).max(1),
		test_labels,
		eval_method=eval_method,
		verbose=verbose,
	)
	...
	results_4_1, df_4_1 = evaluate(
		median_iqr_norm(np.abs(inv_pca - test_array), norm=None).max(1),
		test_labels,
		eval_method=eval_method,
		verbose=verbose,
	)
	...
	# Baseline-6: L2 norm
	results_5, df_5 = evaluate(
		np.linalg.norm(test_array, 2, axis=1),
		test_labels,
		eval_method=eval_method,
		verbose=verbose,
	)
	...
	score_dict = {
		"F1": cf[eval_method].iloc[0].tolist(),
...
def evaluate_datasets(
	...
	for dataset in datasets_ordered:
		...
		if verbose:
			if isinstance(train, list):
				print(
					f" Evaluation on {dataset_name} finished: on {len(train)} traces,  results are averaged"
				)
			else:
